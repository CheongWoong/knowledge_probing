# reference: https://github.com/huggingface/transformers/blob/v4.33.2/examples/pytorch/language-modeling/run_clm.py
...
def main():
    ...
    if custom_args.prompt_tuning:
        ...
        if training_args.do_train:
            peft_config = LoraConfig(task_type=TaskType.CAUSAL_LM, r=16, lora_alpha=16, target_modules=["q_proj", "v_proj"], lora_dropout=0.1, bias="none")
            model = get_peft_model(model, peft_config)
            print(model.print_trainable_parameters())
        else:
            model = PeftModel.from_pretrained(model, peft_model_id)
    ...
    trainer = Trainer(
        ...
        data_collator=data_collator, ##### cwkang: use the custom data collator
