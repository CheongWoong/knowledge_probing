# reference: https://github.com/huggingface/transformers/blob/v4.33.2/examples/pytorch/language-modeling/run_mlm.py
...
import evaluate
import torch
torch.set_num_threads(8) ## cwkang: set the number of CPU threads
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, data_args, training_args, remaining_args = parser.parse_args_into_dataclasses(return_remaining_strings=True) ## cwkang: save the remaining arguments

    ##### cwkang: get the remaining arguments
    import argparse
    def str2bool(v):
    ...
    data_args.max_seq_length = data_args.block_size ## cwkang: match the argument names
    ...
    if 'train' in raw_datasets:
        train_dataset = SupervisedDataset(raw_datasets['train'], tokenizer, max_seq_length, custom_args.truncated_prompt, is_training=True)
    if 'validation' in raw_datasets:
        eval_dataset = SupervisedDataset(raw_datasets['validation'], tokenizer, max_seq_length, custom_args.truncated_prompt)
